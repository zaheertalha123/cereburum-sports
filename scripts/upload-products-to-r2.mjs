...
const LOCAL_ROOT = path.join(ROOT, 'public', 'images', 'products');
const R2_PREFIX = 'images/products';
...
async function main() {
  ...
  if (!localStat?.isDirectory()) {
    throw new Error(`Product media folder not found: ${LOCAL_ROOT}`);
  }

  const files = await walk(LOCAL_ROOT);
  if (files.length === 0) {
    console.log('No product .webp files to upload.');
    return;
  }
  ...
  if (publicBase) {
    console.log(`\nExample URL: ${publicBase}/images/products/${path.basename(files[0])}`);
  }
