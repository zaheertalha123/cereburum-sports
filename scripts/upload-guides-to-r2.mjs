...
const LOCAL_ROOT = path.join(ROOT, 'public', 'images', 'guides');
const R2_PREFIX = 'images/guides';
...
async function main() {
  ...
  if (!localStat?.isDirectory()) {
    throw new Error(`Guide media folder not found: ${LOCAL_ROOT}`);
  }

  const files = await walk(LOCAL_ROOT);
  if (files.length === 0) {
    console.log('No guide files to upload.');
    return;
  }
  ...
  if (publicBase) {
    ...
    for (const filePath of files) {
      const relative = path.relative(LOCAL_ROOT, filePath).split(path.sep).join('/');
      console.log(`  ${publicBase}/images/guides/${relative}`);
    }
  }
